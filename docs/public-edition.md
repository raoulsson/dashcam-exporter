...
`Builder` for item 5 and an `Uploader` for item 7.
...
**Disabled, not hidden.** All nine steps are always in the grid at their own
fixed numbers; item 7 is greyed, and `h7` says what it is —
